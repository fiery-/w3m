...
#include "regex.h"
#ifdef USE_M17N
#include "libwc/wc.h" //Ogi - libwc/
#include "libwc/wtf.h" //Ogi - libwc/
#ifdef USE_UNICODE
#include "libwc/ucs.h" //Ogi - libwc/
#endif
#endif
#ifdef USE_MOUSE
#ifdef USE_GPM
...
//    fprintf(f, "usage: w3m [options] [URL or filename]\noptions:\n");
    fprintf(f, "usage: cat [filename] | w3m [options]\noptions:\n");
...
//    fprintf(f, "    -debug           DO NOT USE\n");
    fprintf(f, "    -endnl count     specify the number of end blank lines\n"); //Ogi - new option
//    if (show_params_p)
//	show_params(f);
    exit(err);
}
...
//	    }
	    else if (!strcmp("-endnl",argv[i])) {
            if (++i >= argc)
                usage();
            EndNL = atoi(argv[i]);
			//fprintf(stderr, "EndNL %d", EndNL);
			//fflush(stderr);
	    }
	    else {
		usage();
...
//		rFrame();
//	}
	if (w3m_dump) do_dump(newbuf); //Ogi
//	    do_dump(Currentbuf);
//	else {
//	    Currentbuf = newbuf;
//#ifdef USE_BUFINFO
