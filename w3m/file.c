...
is_boundary(unsigned char *ch1, unsigned char *ch2)
{
	...
	return 0;
#endif
    if ((*ch1 == '-' || *ch1 == '+' || *ch1 == '.' || IS_DIGIT(*ch1)) && IS_DIGIT(*ch2)) //Ogi - don't break digits
	return 0;
        
    if (get_strwidth(ch2) < 10 && is_word_char(ch1) && is_word_char(ch2)) //Ogi - get_strwidth(ch2) < 10
	return 0;
...
//    if (!parsedtag_get_value(tag, ATTR_SRC, &p)) //Ogi - we will not process images
	return tmp;
    p = remove_space(p);
    q = NULL;
...
HTMLtagproc1(struct parsed_tag *tag, struct html_feed_environ *h_env)
{
    ...
    x = 0;//    x = 2; //Ogi - we will not read cellspacing/cellpadding to save space
    y = 0;//    y = 1;
	z = 0;
	width = 0;
	...
	if (parsedtag_exists(tag, ATTR_HBORDER))
	    w = BORDER_NOWIN;
//	parsedtag_get_value(tag, ATTR_CELLSPACING, &x); //Ogi - we will not read cellspacing/cellpadding to save space
...
_saveBuffer(Buffer *buf, Line *l, FILE * f, int cont)
{
  ...
  pager_next:
	...
	if (Strlastchar(tmp) != '\n' && !(cont && l->next && l->next->bpos))
	    putc('\r', f); //Ogi - added \r - CR+new line for printers
        putc('\n', f);
    }
    ...
    if(EndNL > 0 && EndNL < 100){ //Ogi - add blank lines at the end
        for(int eix = 0; eix < EndNL; eix++){ 
            putc('\r', f);
            putc('\n', f);
        }
    }
